...
static bool isBoolLiteral(const Expression& expression, bool value) {
    if (expression.expressionType != ExpressionType::LITERAL || expression.dataType != LogicalType::BOOL()) {
        return false;
    }
...
std::shared_ptr<Expression> ExpressionBinder::simplifyCaseExpression(
    ...
    for (auto i = 0u; i < caseExpression.getNumCaseAlternatives(); ++i) {
        ...
        if (ExpressionUtil::isNullLiteral(*whenExpression) || isBoolLiteral(*whenExpression, false)) {
            changed = true;
            continue;
        }
