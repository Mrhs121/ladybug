...
static std::unique_ptr<FileInfo> acquireReadOnlyCheckpointApplyLock(
    ...
    // The intent read lock is held only while acquiring the apply read lock. This lets a writer
    // announce checkpoint intent and block new read-only recovery attempts while it waits for
    // existing recoveries to release the apply lock.
    auto intentLock = tryAcquireReadOnlyCheckpointLock(
        clientContext, StorageUtils::getCheckpointIntentLockFilePath(databasePath));
    auto applyLock = tryAcquireReadOnlyCheckpointLock(
